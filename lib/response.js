...
var Response = module.exports = Compose(function(res, app) {
    ...
    conference: function(room, opts) {
        if (this._app) {
            (opts.callback || opts.callbackUrl) && (opts.callbackUrl = this._app._getCallbackUrl(opts.callback || opts.callbackUrl));
            opts.action && (opts.action = this._app._getCallbackUrl(opts.action));
        }
...
function _add(res, type, attrs, body) {
    ...
    if (body) {
        buf.push('>');
        buf.push(_xmlEncode(typeof body === 'function' ? body(res) : body));
        buf.push('</', type, '>');
    } else {
        buf.push(' />');
    }
